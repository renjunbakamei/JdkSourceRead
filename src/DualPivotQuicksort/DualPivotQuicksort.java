...
public class DualPivotQuicksort {
    ...
    public static void sort(char[] a,int left,int right,
        ...
        //待排序长度大于这个值，使用快速排序
        if(right - left<Common.COUNTING_SORT_THRESHOLD_FOR_SHORT_OR_CHAR){
            int[] count=new int[Common.NUM_CHAR_VALUES];

            for(int i=left-1;++i<=right; count[a[i]]++);

            for(int i=Common.NUM_CHAR_VALUES,k=right+1;k>left;){
                while (count[--i]==0);
                char value=(char)i;
                int s=count[i];
    ...
    private static void doSort(char[] a, int left, int right,
        ...
        if (right - left < Common.QUICKSORT_THRESHOLD) {
         ...
         */
        int[] run = new int[Common.MAX_RUN_COUNT + 1];
        ...
        for (int k = left; k < right; run[count] = k) {
            if (a[k] < a[k + 1]) { // ascending
                ...
                for (int m = Common.MAX_RUN_LENGTH; ++k <= right && a[k - 1] == a[k]; ) {
                    if (--m == 0) {
                        sort(a, left, right, true);
                        return;
                    }
    ...
    private static void sort(char[] a, int left, int right, boolean leftmost) {
        ...
        if (length < Common.INSERTION_SORT_THRESHOLD) {
            if (leftmost) {
                /*
